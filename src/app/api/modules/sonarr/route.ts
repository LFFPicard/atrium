...
const ALLOWED = new Set(['calendar', 'series', 'image']);
...
export async function GET(req: NextRequest) {
  ...
  if (!isModuleEnabled('sonarr')) {
    return NextResponse.json({ error: 'Sonarr module is not enabled' }, { status: 503 });
  }
  ...
  const record = getModule('sonarr');
  const sonarrBase = base(record.config['sonarr_url'] ?? '');
  const key = record.config['sonarr_api_key'] ?? '';
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: `Sonarr returned ${res.status}` }, { status: 502 });
    }
